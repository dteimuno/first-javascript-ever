/*Let’s construct a promise! To create a new
Promise
Preview: Docs Loading link description
object, we use the new keyword and the Promise constructor method:
*/
...
promise is useful, but most of the time, knowing how to consume, or use, promises will be key. Rather than constructing promises, we often need to handle Promise
objects returned to us as the result of an asynchronous operation. These promises will start off pending, but eventually settle.

Moving forward, we’ll be simulating this by providing functions that return promises that settle after some time. To accomplish this, we’ll be using
setTimeout()
...
/*
In the example code, we invoked returnPromiseFunction(), which returns a promise. We assigned that promise to the variable prom. Similar to the asynchronous promises we may encounter in production code, prom will initially have a status of pending.
...
objects come with an aptly named .then() method. It allows us to say, “I have a promise, when it settles, then here’s what I want to happen…”
...
functions as arguments. We refer to these callbacks as handlers. When the promise settles, the appropriate handler will be invoked with that settled value.
...
Let’s break down what’s happening in the example code:
    ...
    functions
    Preview: Docs Loading link description
    to .then(). The first will be invoked with 'Yay!' if the promise resolves, and the second will be invoked with 'Ohhh noooo!' if the promise rejects.
